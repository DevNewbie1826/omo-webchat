...
describe("ChatPane status row", () => {
  ...
  it.each([0, 42])("exposes metrics at rest, including %s, without a disclosure", (percent) => {
    const { deliver } = renderChatPane(root);
    act(() => deliver({ type: "stats", sessionId: "chat-1",
      contextUsage: { tokens: percent, contextWindow: 100, percent },
      tokens: { input: 30, cacheRead: 70, output: 5 } }));
    const status = requireElement(container.querySelector(".th-chat-status"), "status");
    expect(status.querySelector("details, summary")).toBeNull();
    expect([...status.querySelectorAll(".th-chat-status-num")].map(n => n.textContent)).toEqual([`${percent}%`, "70%"]);
  });

  it("keeps a single stable run slot through idle, running and compaction", () => {
    const { deliver } = renderChatPane(root);
    const slot = requireElement(container.querySelector("[data-chat-run-state]"), "stable run slot");
    expect(slot.getAttribute("data-chat-run-state")).toBe("idle");
    act(() => deliver({ type: "run.started", sessionId: "chat-1" }));
    expect(container.querySelector("[data-chat-run-state]")).toBe(slot);
    expect(slot.getAttribute("data-chat-run-state")).toBe("responding");
    expect(slot.getAttribute("aria-label")).toBe(i18n.t("chat.responding"));
    expect(slot.getAttribute("title")).toBe(i18n.t("chat.responding"));
    expect(slot.textContent).toBe("");
    act(() => deliver({ type: "compaction.started", sessionId: "chat-1" }));
    expect(container.querySelectorAll(".th-chat-status-spinner")).toHaveLength(1);
    expect(container.querySelector(".th-chat-status")?.textContent).toContain("chat.compacting");
    act(() => deliver({ type: "run.done", sessionId: "chat-1", reason: "stop" }));
    expect(slot.getAttribute("data-chat-run-state")).toBe("idle");
    expect(slot.querySelector(".th-chat-status-spinner")).toBeNull();
  });

  it("keeps compaction warnings before variable previews without hiding metrics", () => {
    const { deliver } = renderChatPane(root);
    const input = requireElement(container.querySelector<HTMLTextAreaElement>("textarea"), "composer");
    ...
    const primary = requireElement(status.querySelector(".th-chat-status-primary"), "primary states");
  ...
  it("focuses a wrapped original preview without programmatic ancestor scrolling", () => {
    renderChatPane(root);
    const input = requireElement(container.querySelector<HTMLTextAreaElement>("textarea"), "composer");
    ...
    expect(primary.scrollLeft).toBe(0);
