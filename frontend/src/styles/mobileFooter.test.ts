...
describe("mobile sidebar footer bounds", () => {
  ...
  test("uses visual height and resets the root-contained standalone sidebar origin once", () => {
    expect(baseSidebarBody).toContain("height: calc(var(--th-vh-unit, 1vh) * 100 - env(safe-area-inset-top))");
    const standaloneBody = sidebarCss.match(
      /^html\[data-th-standalone\] \.th-sidebar \{([^}]*)\}/m,
    )?.[1] ?? "";
    expect(standaloneBody).toContain("top: 0");
