...
describe("mobile sidebar footer bounds", () => {
  ...
  test("extends the standalone sidebar over the installed app's full screen while the keyboard is closed", () => {
    // Mirrors #root's standalone gate: the dynamic viewport ends above the
    // home-indicator gesture zone in installed PWAs, so both sidebar
    // renderings must consume the large viewport (keyboard closed) while
    // their existing safe insets and the keyboard/ordinary-browser policies
    // stay untouched.
    const standaloneBody = sidebarCss.match(
      /^html\[data-th-standalone\]:not\(\[data-th-keyboard-open\]\) \.th-sidebar \{([^}]*)\}/m,
    )?.[1] ?? "";
    expect(standaloneBody).toContain("height: calc(100vh - env(safe-area-inset-top))");
    expect(standaloneBody).toContain("height: calc(100lvh - env(safe-area-inset-top))");
    const mobileBlock = sidebarCss.match(/@media \(max-width: 768px\) \{([\s\S]*?)\n\}/)?.[1] ?? "";
    const standaloneDrawer = mobileBlock.match(
      /html\[data-th-standalone\]:not\(\[data-th-keyboard-open\]\) \.th-sidebar \{([^}]*)\}/,
    )?.[1] ?? "";
    expect(standaloneDrawer).toContain("height: 100lvh");
  });
