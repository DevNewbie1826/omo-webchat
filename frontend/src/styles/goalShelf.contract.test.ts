...
describe("goal shelf shrink contract", () => {
  ...
  it("makes .th-activity-shelf a shrinkable flex column floored at the tab-strip height (never flex: none)", () => {
  ...
  it("keeps the activity tab strip non-shrinking with flex: none", () => {
  ...
  it("reserves the grip and a minimal panel box in the expanded activity-shelf floor", () => {
    const expanded = ruleBody(
      activityShelf,
      '.th-activity-shelf[data-expanded="true"]',
    );
    ...
    // on the data-expanded rule, not the base shelf.
    ...
    if (compactCss(collapsed) !== ACTIVITY_COLLAPSED_HEIGHT_FLOOR) {
      violations.push(
        `activity-shelf.css collapsed .th-activity-shelf min-height is "${collapsed || "missing"}"; ` +
          "collapsed must keep the tab-strip floor (no blank space below the chrome)",
      );
    }
  ...
  it("draws the activity tab focus outline inside the clipped shelf", () => {
    const focus = ruleBody(activityShelf, ".th-activity-tab:focus-visible");
    const violations: string[] = [];
    if (declarationValue(focus, "outline-offset") !== "-2px") {
      violations.push(
        `activity-shelf.css .th-activity-tab:focus-visible outline-offset is ` +
          `"${declarationValue(focus, "outline-offset") || "missing"}"; expected -2px`,
      );
    }
    expect(violations).toEqual([]);
