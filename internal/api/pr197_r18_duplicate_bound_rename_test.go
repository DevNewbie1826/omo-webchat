...
type pr197R18RenameGate struct {
	*wsbridge.CursorStore
	entered chan struct{}
	release chan struct{}
}

func (s *pr197R18RenameGate) UpdateName(ctx context.Context, id, name, source string) error {
	if name == "After" {
		close(s.entered)
		select {
		case <-s.release:
...
func TestPR197R18BoundRenameWithDuplicateDurableClaim(t *testing.T) {
	...
	gate := &pr197R18RenameGate{CursorStore: (*wsbridge.CursorStore)(st.Store), entered: make(chan struct{}), release: make(chan struct{})}
	...
	// A newer stored claimant wins the durable lookup, but the live route
	// still belongs to bound-rename and must use that chat's stored name.
	duplicate := cursorstore.Chat{
		ID: "duplicate-claim", WorkspaceID: workspace.ID, CWD: workspace.Path,
		Name: "Duplicate", NameSource: cursorstore.NameSourceUser,
		DurableSessionID: sess.ID(), LastUsedAt: 4102444800000,
	}
	if err := st.SaveChat(duplicate); err != nil {
		t.Fatal(err)
	}
	if owner, ok := st.ChatForDurable(sess.ID()); !ok || owner.ID != duplicate.ID {
		t.Fatalf("duplicate fixture did not win durable lookup: %+v, %t", owner, ok)
	}
