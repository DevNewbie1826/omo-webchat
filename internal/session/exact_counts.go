...
func (s *Session) refreshExactCountsLocked() (running, total int) {
	...
	publishLiveCounts(s.taskDigest, s.dagDigest, liveCountCaches{&s.taskSnapshots, &s.dagSnapshots})
	return running, total
}
...
func refreshOverviewExactCounts(entry *overviewCacheEntry) {
	...
	publishLiveCounts(entry.task, entry.dag, liveCountCaches{&entry.taskSnapshots, &entry.dagSnapshots})
}
